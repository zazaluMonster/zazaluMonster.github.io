 ...
 * 
 * 不可变任意精度整数, 任意精度表示的意思是可以是任何位数的, 也就是说可以对数字进行精确表示, 不会损失精度
 * 
 * All operations behave as if BigIntegers were represented in two's-complement notation (like Java's
 * primitive integer types).  
 * 
 * BigInteger的所有操作都表现为操作二进制补码(操作byte[])
 * 
 ...
 * 
 * 
 * BigInteger类提供了所有的整数运算符对应的方法操作,以及java.lang.Math中的所有相关方法(其实意思就是说你需要的计算方法我都有)
 * 此外我还提供了modular arithmetic, GCD calculation, primality testing, prime generation, bit manipulation和其他杂七杂八的操作
 *
 ...
 * 
 * 位操作除了满足一般的位操作形式外,还支持负移位距离. 右移位+负移位 = 正常的左移位, 反之亦然
 * 无符号右移位(>>>)会被忽略,因为它与此类的"无限字长"的抽象结合起来没有意义
 *
 ...
 * 
 * 类常量private static final  int PRIME_SEARCH_BIT_LENGTH_LIMIT = 500000000
 * 跟这个注释有关
 *
 * @implNote
 ...
 * +2<sup>{@code Integer.MAX_VALUE}</sup> (exclusive).
 * 
 * BigInteger目前的实际取值范围是 -2的Integer.MAX_VALUE次方 (不可兼) 到 +2的Integer.MAX_VALUE次方 (不可兼).是被规定的一个大小
 * 不过由于BigInger的底层有int[]实现, 所以理论取值范围是非常大的, 因为一个int是32位, 而java的数组最大长度是[int的最大值-5], 为什么是-5是因为java底层实现这5位存了其他元数据占用了.
 * 所以BigInteger可以使用的二进制位数就是32*[int的最大值-5] = 68719476544位(所以实际能表示的范围是巨大的,比int的最大值的位数还大), 转换为我们熟悉的容量单位就是差不多8191.9999771118GB! 这个数字毫无实际意义- -,所以我估测是因为安全起见,所以BigInteger自己定义了一个取值范围的限制
 * 
 * 
 * @see     BigDecimal
 * @author  Josh Bloch
...
public class BigInteger extends Number implements Comparable<BigInteger> {
     ...
     * 
     * 另一个控制取值范围的玩意,好像是控制质数范围1~2的500000000次方
     */
